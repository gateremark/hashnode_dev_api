...
import { UserQueryResponse, Edge, Badge } from "./UserInfo.types";
...
const ShowInfo = ({ data }: ShowInfoProps) => {
  ...
  const posts = data.user.posts.edges; // This is now properly typed as Edge[]
  const location = data.user.location;

  // Handling potential undefined values
  const github = data.user.socialMediaLinks.github
    ? data.user.socialMediaLinks.github.split("/").pop()
    : "";
  const twitter = data.user.socialMediaLinks.twitter
    ? data.user.socialMediaLinks.twitter.split("/").pop()
    : "";
  ...
  return (
    <div className="md:grid lg:grid-cols-2 w-full h-full grid-cols-1">
      ...
      <div className=" flex flex-col items-center justify-center">
        <h1 className="text-6xl poor__font text-[#ffffff]"> Latest Blogs</h1>
        <div className="grid grid-cols-2 p-4 gap-4">
          {posts.length === 0 && (
            <p className="text-lg text-[#ffffff]">No posts yet 😔</p>
          )}
          {posts.map((p: Edge) => (
            <PostItem key={p.node.title} post={p.node} />
          ))}
        </div>
      </div>
      ...
      <Tilt className="w-[450px] z-50">
        <div
          className="flex flex-col items-center justify-center w-[450px] sm:h-[100vh] h-[81vh] text-[#ffffff] pt-4"
          ref={elementRef}
        >
          <div
              ...
              <div className=" z-10">
                ...
                >
                  ...
                  <div className="flex gap-2 mt-[2px] flex-wrap">
                    {badges.length === 0 && (
                      <p className="text-base text-slate-500">No badges yet</p>
                    )}
                    {badges.length !== 0 &&
                      badges
                        .slice(0, 8)
